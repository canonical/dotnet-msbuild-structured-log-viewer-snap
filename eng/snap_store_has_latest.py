...
def get_snap_store_version(channel):
    ...
    try:
        ...
        conn.request("GET", "/v2/find?name=msbuild-structured-log-viewer")
        ...
        if data.get("type") == "sync" and "result" in data:
            ...
            # Find msbuild-structured-log-viewer snap (exact match)
            for snap in results:
                if snap.get("name") == "msbuild-structured-log-viewer":
            ...
            print("msbuild-structured-log-viewer snap not found in store", file=sys.stderr)
            return None
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check if Snap Store has the latest msbuild-structured-log-viewer version " \
        "for a given channel"
    )
    parser.add_argument(
        "--channel",
    ...
    if expected_version == snap_version:
        print(f"✓ Versions match - Snap Store {args.channel} has the latest " \
              "msbuild-structured-log-viewer version")
        return 0  # Versions match

    print(f"✗ Versions differ - Snap Store {args.channel} does NOT have the latest " \
          "msbuild-structured-log-viewer version")
